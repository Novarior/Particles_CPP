...
public:
  ParticleSystem(unsigned int count, sf::Vector2f size_window)
      : m_particles(count), m_vertices(sf::PrimitiveType::Points, count),
        m_lifetime(5.f), m_emitter(0.f, 0.f), m_size_window(size_window / 2.f) {
    m_draw_mode = 0;
  }
  ...
  float get_zeroEmitterLifeTime() const { return m_particles[0].lifetime; }

  // get count size
  size_t get_size() const { return m_particles.size(); }

  // get alpha of particle[0]
  std::uint8_t get_alpha() const { return m_vertices[0].color.a; }
  ...
  void update(float elapsed) {
    for (std::size_t i = 0; i < m_particles.size(); ++i) {
      ...
      if (m_draw_mode == 0 || m_draw_mode == 1 || m_draw_mode == 2 ||
          m_draw_mode == 5 || m_draw_mode == 6 || m_draw_mode == 8) {
        if (r < 0 && m_draw_mode == 1)
          r = -r;
        else if (r < 0 && m_draw_mode == 8)
          r = 0;
        m_vertices[i].color.r = static_cast<std::uint8_t>(r);
      } else if (m_draw_mode == 3 || m_draw_mode == 4 || m_draw_mode == 7) {
        m_vertices[i].color.r = 0;
      }

      // draw green color if mode 0, 1, 3, 5, 7, 8
      if (m_draw_mode == 0 || m_draw_mode == 1 || m_draw_mode == 3 ||
          m_draw_mode == 5 || m_draw_mode == 7 || m_draw_mode == 8) {
        if (g < 0 && m_draw_mode == 1)
          g = -g;
        if (g < 0 && m_draw_mode == 8)
          g = 0;
        m_vertices[i].color.g = static_cast<std::uint8_t>(g);
      } else if (m_draw_mode == 2 || m_draw_mode == 4 || m_draw_mode == 6) {
        m_vertices[i].color.g = 0;
      }

      // draw blue color if mode 0, 1, 4, 6, 7, 8
      if (m_draw_mode == 0 || m_draw_mode == 1 || m_draw_mode == 4 ||
          m_draw_mode == 6 || m_draw_mode == 7 || m_draw_mode == 8) {
        if (b < 0 && m_draw_mode == 1)
          b = -b;
        if (b < 0 && m_draw_mode == 8)
          b = 0;
        m_vertices[i].color.b = static_cast<std::uint8_t>(b);
      } else if (m_draw_mode == 2 || m_draw_mode == 3 || m_draw_mode == 5) {
        m_vertices[i].color.b = 0;
      }

      // reverse alpha
      m_vertices[i].color.a = static_cast<std::uint8_t>(
          255 - (255 * sin(M_PI + ratio * M_PI)) - 255);
    }
  }
